...
export type InputViewOnFilterButtonClickedHandler = (filtertype: Filter) => void;
...
export class InputView extends React.Component<Props, State> {
    ...
    protected handleChange: React.ChangeEventHandler<HTMLInputElement> = event => {
        this.setState({
    ...
    protected submitNewItem: React.MouseEventHandler<HTMLButtonElement> = event => {
    ...
    protected handleKeyboardPress: React.KeyboardEventHandler<HTMLInputElement> = event => {
        if (event.key === "Enter") {
            if (this.isInputEmpty(this.state.inputValue)) {
                return;
            }
    ...
    protected onUncompletedClick: React.MouseEventHandler<HTMLButtonElement> = event => {
        this.props.onFilterButtonClicked(Filter.Uncompleted);
    };

    protected onCompletedClick: React.MouseEventHandler<HTMLButtonElement> = event => {
        this.props.onFilterButtonClicked(Filter.Completed);
    };

    protected onShowAllClick: React.MouseEventHandler<HTMLButtonElement> = event => {
